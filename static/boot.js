...
async function cancelStream(){
  ...
  if(!streamId) return;
  try{
    await fetch((window.__HERMES_REMOTE_URL||'')+'/'+`api/chat/cancel?stream_id=${encodeURIComponent(streamId)}`,{credentials:'include'});
...
(function(){
  ...
  async function _transcribeBlob(blob){
    ...
    try{
      const res=await fetch((window.__HERMES_REMOTE_URL||'')+'/api/transcribe',{method:'POST',body:form});
